...
const publicationAuthorLinks = {
  ...
  "Wonjae Lee": "https://smc.skku.edu/doctor/main/main.do?mId=1&medDrSeq=286",
...
function linkPublicationAuthors() {
  document.querySelectorAll(".publication-authors").forEach((element) => {
    const authors = element.innerHTML.split(/,\s*/);
    element.replaceChildren();

    authors.forEach((markup, index) => {
      if (index > 0) element.append(", ");

      const template = document.createElement("template");
      template.innerHTML = markup;
      const name = template.content.textContent.replace(/†/g, "").trim();
